...
import org.wso2.carbon.idp.mgt.stub.dto.SAMLSSOIdentityProviderDTO;
...
public class IdentityProviderMgtServiceClient {
    ...
    public boolean addMetadataServiceProviderFileContentOnly(String fileContent, String issuer) throws AxisFault {

        boolean status = false;

        try {
            status = idPMgtStub.isMetadataFileAdded(fileContent, issuer);


        } catch (Exception e) {
            log.error("Error adding a new Identity Provider file 1", e);
    ...
    public SAMLSSOIdentityProviderDTO addMetadataServiceProviderSAMLObjectOnlyFromFile(String fileContent) throws AxisFault {

        SAMLSSOIdentityProviderDTO samlssoIdentityProviderDTO;
        try {
            samlssoIdentityProviderDTO = idPMgtStub.addMetadataServiceProviderFromFile(fileContent);
        } catch (Exception e) {
            log.error("Error adding a new identity Provider file 2", e);
            throw new AxisFault(e.getMessage(), e);
        }
