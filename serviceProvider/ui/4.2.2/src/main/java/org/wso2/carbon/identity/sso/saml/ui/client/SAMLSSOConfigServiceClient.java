...
public class SAMLSSOConfigServiceClient {
    ...
    public org.wso2.carbon.identity.core.model.xsd.SAMLSSOServiceProviderDO addMetadataServiceProviderFromFile(String fileContent) throws AxisFault {
        System.out.println("enter the class SAMLSSOClient");

        try {
            System.out.println("enter the class SAMLSSOClient try block");
            return stub.addMetadataServiceProvider(fileContent);

        } catch (Exception e) {
            log.error("Error Converting a new Service Provider object", e);
            throw new AxisFault(e.getMessage(), e);
        }

    }

    public boolean addMetadataSPResource(String fileContent) throws AxisFault {

        try {
           return stub.isMetadataFileAdded(fileContent);
        } catch (Exception e) {
            log.error("Error adding service provider matadata resource", e);
            throw new AxisFault(e.getMessage(), e);
        }
    ...
    public boolean addMetadataSPObject(org.wso2.carbon.identity.core.model.xsd.SAMLSSOServiceProviderDO samlssoServiceProviderDO) throws AxisFault {

        try {
            return stub.isMetadataFileSAMLObjectAdded(samlssoServiceProviderDO);
        } catch (Exception e) {
            log.error("Error adding service provider matadata resource", e);
            throw new AxisFault(e.getMessage(), e);
        }
