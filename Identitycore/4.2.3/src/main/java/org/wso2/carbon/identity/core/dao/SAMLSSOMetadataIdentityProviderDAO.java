...
public class SAMLSSOMetadataIdentityProviderDAO extends AbstractDAO {
    private static final Log log = LogFactory.getLog(SAMLSSOMetadataServiceProviderDAO.class);

    public SAMLSSOMetadataIdentityProviderDAO(Registry registry) {
     ...
     * @param idpEntityId name of the issuer
     * @throws org.wso2.carbon.identity.base.IdentityException
     */
    public boolean addMetadataIdentityProvider(String fileContent, String idpEntityId) throws IdentityException {
        String path = null;
        Resource resource;


        if (fileContent != null && idpEntityId != null) {

            path = IdentityRegistryResources.SAML_SSO_METADATA_IDENTITY_PROVIDERS_INBOUND + encodePath(idpEntityId);
        }
        ...
        try {
            ...
            throw new IdentityException("Error while adding Identity Provider metadata file", e);
        }

        if (log.isDebugEnabled()) {
            log.debug("Identity Provider metadata file is added successfully.");
        }
        return true;
    }
...
//    }
    ...
    private String encodePath(String idpName) {
        String encodedStr = new String(Base64.encodeBase64(idpName.getBytes()));
        return encodedStr.replace("=", "");
    }

    @Override
    protected Object resourceToObject(Resource resource) {
        return null;
    }
}
